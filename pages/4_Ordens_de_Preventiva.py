...
def _load_prev_record(engine, prev_id: int):
    df = fetch_df(engine, """
        SELECT id, id_maquina, id_funcionario, datahora_inicio, datahora_fim,
               servico_realizado, status FROM ordens_preventiva WHERE id=:id
    """, {"id": int(prev_id)})
    return None if df.empty else df.iloc[0].to_dict()

def _inject_edit_state_prev(rec, id2maqname, id2funcname):
    ss = st.session_state
    ss["_prefill_prev_maquina_name"] = id2maqname.get(int(rec["id_maquina"]), "") if rec.get("id_maquina") else ""
    ss["_prefill_prev_resp_name"]    = id2funcname.get(int(rec["id_funcionario"]), "—") if rec.get("id_funcionario") else "—"
    ss["_prefill_prev_servico"] = str(rec.get("servico_realizado") or "")
    ss["_prefill_prev_status"]  = str(rec.get("status") or "Aberta")
    ss["_prefill_prev_dti"] = _as_date(rec.get("datahora_inicio"))
    ss["_prefill_prev_ti"]  = _as_time(rec.get("datahora_inicio"))
    ss["_prefill_prev_dtf"] = _as_date(rec.get("datahora_fim"))
    ss["_prefill_prev_tf"]  = _as_time(rec.get("datahora_fim"))
...
engine = get_engine()
T, T_MO = TABLES["prev"], TABLES["mo_prev"]
...
if "prev_edit_id" not in st.session_state: st.session_state["prev_edit_id"] = None
if "prev_ed_loaded_for" not in st.session_state: st.session_state["prev_ed_loaded_for"] = None

# Nova Preventiva
st.subheader("➕ Nova Preventiva")
if can("create_prev"):
    c1, c2, c3 = st.columns(3)
    with c1:
        nome_maquina_new = st.selectbox("Máquina", MAQ_OPTS, key="prev_new_maquina") if MAQ_OPTS else st.selectbox("Máquina", [], key="prev_new_maquina_empty")
        ...
        if st.button("💾 Salvar nova Preventiva", type="primary", use_container_width=True, key="prev_btn_save_new"):
            ...
            if not maquina_escolhida:
                ...
                if dt_fim < dt_ini:
                    ...
                    st.success("Preventiva inserida!"); st.rerun()
else:
    st.info("Seu perfil não permite criar Preventiva.")

st.divider()

# Editar Preventiva
st.subheader("✏️ Editar Preventiva")
df_sel = fetch_df(engine, """
    SELECT p.id, m.nome AS maquina, p.datahora_inicio
    FROM ordens_preventiva p JOIN maquinas m ON m.id = p.id_maquina
    ORDER BY p.datahora_inicio DESC, p.id DESC LIMIT 200
""", {})

if df_sel.empty:
    st.info("Não há preventivas para editar.")
    st.session_state["prev_edit_id"] = None; st.session_state["prev_ed_loaded_for"] = None
else:
    ...
    opt_labels = df_sel.apply(
        ...
        axis=1).tolist()
    idx_default = opt_values.index(int(st.session_state["prev_edit_id"])) if st.session_state.get("prev_edit_id") in opt_values else 0
    sel_id_quick = st.selectbox("🔎 Selecionar Preventiva para edição", options=opt_values,
                                format_func=lambda v: opt_labels[opt_values.index(v)],
                                index=idx_default if len(opt_values) else 0, key="prev_quick_select")
...
if not rec_ok:
    ...
    with c1e:
        ...
        nome_maquina_ed = st.selectbox("Máquina", MAQ_OPTS or [""], index=idx_maq if MAQ_OPTS else 0, key="prev_ed_maquina_name")
    ...
    with cE1:
        if can("edit_prev") and st.button("💾 Salvar edições", type="primary", use_container_width=True, key="prev_btn_save_edit"):
            ...
            if dt_fim < dt_ini: st.error("Hora fim não pode ser menor que hora início.")
                ...
                maquina_ed = st.session_state.get("prev_ed_maquina_name")
                ...
                if rec: _inject_edit_state_prev(rec, id2maqname, id2funcname); _apply_prev_prefill_to_widgets()
                st.success("Preventiva alterada com sucesso!")
    with cE2:
        if can("delete_prev") and st.button("🗑️ Excluir Preventiva", type="secondary", use_container_width=True, key="prev_btn_delete"):
            prev_id = int(st.session_state["prev_edit_id"])
            delete_where(engine, T_MO, "id_ordem_preventiva = :pid", {"pid": prev_id})
            delete_by_ids(engine, T, [prev_id])
            st.success(f"Preventiva #{prev_id} excluída (com MOs).")
            st.session_state["prev_edit_id"] = None; st.session_state["prev_ed_loaded_for"] = None; st.rerun()

st.divider()
st.subheader("🧑‍🔧 Mão de Obra da Preventiva selecionada")

prev_id_ref = st.session_state.get("prev_edit_id")
try: prev_id_ref = int(prev_id_ref) if prev_id_ref is not None else None
except: prev_id_ref = None
...
if not prev_id_ref:
        ...
        with st.form("form_mo_prev_inline"):
            ...
            if submitted:
                if func_id is None: st.error("Selecione o funcionário.")
                    ...
                    insert_row(engine, T_MO, {"id_ordem_preventiva": int(prev_id_ref),
    ...
    df_mo = fetch_df(engine, """
        ...
        FROM mao_obra_preventiva mo JOIN funcionarios f ON f.id = mo.id_funcionario
        WHERE mo.id_ordem_preventiva = :id ORDER BY mo.data ASC, mo.hora_inicio ASC
    """, {"id": int(prev_id_ref)})
    if df_mo.empty:
        ...
        for _, row in df_mo.iterrows():
            ...
            with st.expander(f"MO #{mo_id} • {func_nome_row} • {row['data']} • {str(row['hora_inicio'])}–{str(row['hora_fim'])}", expanded=False):
                ...
                with c1:
                    novo_func = st.selectbox("Funcionário", FUNC_OPTS, index=idx_func, key=f"moprev_edit_func_{mo_id}", disabled=not can("edit_prev"))
                with c2:
                    nova_data = st.date_input("Data", value=pd.to_datetime(str(row["data"])).date(),
                                              key=f"moprev_edit_data_{mo_id}", disabled=not can("edit_prev"))
                with c3:
                    novo_hi = st.time_input("Hora início", value=_as_time(row["hora_inicio"]),
                                            key=f"moprev_edit_hi_{mo_id}", disabled=not can("edit_prev"))
                with c4:
                    novo_hf = st.time_input("Hora fim", value=_as_time(row["hora_fim"]),
                                            key=f"moprev_edit_hf_{mo_id}", disabled=not can("edit_prev"))
                cA, cB = st.columns(2)
                with cA:
                    if can("edit_prev") and st.button("💾 Salvar", key=f"moprev_btn_save_{mo_id}", use_container_width=True):
                ...
                with cB:
                    if can("delete_prev") and st.button("🗑️ Excluir", key=f"moprev_btn_del_{mo_id}", use_container_width=True):
...
if f_maquina != "Todas" and f_maquina in name2maqid: where.append("AND p.id_maquina = :m"); params["m"] = name2maqid[f_maquina]
if f_status: where.append("AND p.status LIKE :st"); params["st"] = f"%{f_status}%"
if f_dt_ini: where.append("AND p.datahora_inicio >= :di"); params["di"] = datetime(f_dt_ini.year, f_dt_ini.month, f_dt_ini.day, 0,0,0)
if f_dt_fim: where.append("AND p.datahora_inicio <= :df"); params["df"] = datetime(f_dt_fim.year, f_dt_fim.month, f_dt_fim.day, 23,59,59)

total = count_rows(engine, T + " p", " " + " ".join(where), params)
...
df_list = fetch_df(engine, f"""
    SELECT p.id, m.nome AS maquina, f.nome AS responsavel, p.datahora_inicio, p.datahora_fim, p.servico_realizado, p.status
    FROM {T} p JOIN maquinas m ON m.id = p.id_maquina
    LEFT JOIN funcionarios f ON f.id = p.id_funcionario
    {" " + " ".join(where)} ORDER BY p.datahora_inicio DESC, p.id DESC
    LIMIT :lim OFFSET :off
""", {**params, "lim": int(st.session_state["prev_page_size"]), "off": int(offset)})
...
if df_list.empty: st.info("Nada encontrado com os filtros.")
    ...
    st.download_button("⬇️ Exportar CSV", df_list.to_csv(index=False).encode("utf-8"),
                       "ordens_preventiva.csv", "text/csv", use_container_width=True, key="prev_btn_csv")
